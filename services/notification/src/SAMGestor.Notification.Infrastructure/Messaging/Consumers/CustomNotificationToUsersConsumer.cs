...
public sealed class CustomNotificationToUsersConsumer(
    RabbitMqOptions opt,
    RabbitMqConnection conn,
    ILogger<CustomNotificationToUsersConsumer> logger,
    IServiceProvider sp
) : BackgroundService
{
    private const string QueueName = "notification.custom.users";
    private static readonly JsonSerializerOptions JsonOpts = new() { PropertyNameCaseInsensitive = true };

    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        logger.LogInformation("CustomNotificationToUsersConsumer starting…");
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                ...
                await channel.QueueBindAsync(QueueName, opt.Exchange, EventTypes.CustomNotificationToUsersRequestedV1, cancellationToken: stoppingToken);

                await channel.BasicQosAsync(0, 5, false, stoppingToken);
                logger.LogInformation("CustomNotificationToUsersConsumer listening on {queue}", QueueName);
                ...
                while (!stoppingToken.IsCancellationRequested)
                {
                    ...
                    try
                    {
                        ...
                        var env = JsonSerializer.Deserialize<EventEnvelope<CustomNotificationToUsersRequestedV1>>(json, JsonOpts);
                    ...
                    {
                        logger.LogError(ex, "Error processing custom notification to users. Tag={tag}", delivery.DeliveryTag);
            ...
            {
                logger.LogError(ex, "CustomNotificationToUsersConsumer loop error. Retry 5s…");
                await Task.Delay(TimeSpan.FromSeconds(5), stoppingToken);
            }
        }
        logger.LogInformation("CustomNotificationToUsersConsumer stopped.");
    }

    private async Task HandleAsync(CustomNotificationToUsersRequestedV1 evt, CancellationToken ct)
    {
        ...
        foreach (var recipient in evt.Recipients)
        {
            try
            {
                ...
                var message = new NotificationMessage(
                    ...
                    recipientPhone: null,
                    templateKey: "custom-notification",
                ...
                logger.LogInformation(
                    "Custom notification sent: {NotificationId} → {Email}",
            ...
            catch (Exception ex)
            {
                ...
                logger.LogError(ex,
                    "Failed to send custom notification: {NotificationId} → {Email}",
                    evt.NotificationId, recipient.Email);
            }
        }
        ...
        logger.LogInformation(
            "Custom notification batch completed: {NotificationId}, Success={success}, Failed={failed}",
