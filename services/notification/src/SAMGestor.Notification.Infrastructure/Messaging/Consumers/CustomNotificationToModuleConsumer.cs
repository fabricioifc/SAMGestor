...
public sealed class CustomNotificationToModuleConsumer(
    RabbitMqOptions opt,
    RabbitMqConnection conn,
    ILogger<CustomNotificationToModuleConsumer> logger,
    IServiceProvider sp
) : BackgroundService
{
    private const string QueueName = "notification.custom.module";
    private static readonly JsonSerializerOptions JsonOpts = new() { PropertyNameCaseInsensitive = true };

    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        logger.LogInformation("CustomNotificationToModuleConsumer starting…");
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                ...
                await channel.QueueBindAsync(QueueName, opt.Exchange, EventTypes.CustomNotificationToModuleRequestedV1, cancellationToken: stoppingToken);

                await channel.BasicQosAsync(0, 3, false, stoppingToken); // Menor QoS para batch
                logger.LogInformation("CustomNotificationToModuleConsumer listening on {queue}", QueueName);
                ...
                while (!stoppingToken.IsCancellationRequested)
                {
                    ...
                    try
                    {
                        ...
                        var env = JsonSerializer.Deserialize<EventEnvelope<CustomNotificationToModuleRequestedV1>>(json, JsonOpts);
                    ...
                    {
                        logger.LogError(ex, "Error processing custom notification to module. Tag={tag}", delivery.DeliveryTag);
            ...
            {
                logger.LogError(ex, "CustomNotificationToModuleConsumer loop error. Retry 5s…");
                await Task.Delay(TimeSpan.FromSeconds(5), stoppingToken);
            }
        }
        logger.LogInformation("CustomNotificationToModuleConsumer stopped.");
    }

    private async Task HandleAsync(CustomNotificationToModuleRequestedV1 evt, CancellationToken ct)
    {
        ...
        logger.LogInformation(
            "Processing module notification: {NotificationId}, Module={Module}, Recipients={Count}",
            evt.NotificationId, evt.TargetModule, evt.Recipients.Count);

        foreach (var recipient in evt.Recipients)
        {
            try
            {
                var bodyHtml = BuildCustomNotificationTemplate(evt.Template, recipient.Name);
                ...
                var message = new NotificationMessage(
                    ...
                    recipientPhone: null,
                    templateKey: "custom-notification-module",
            ...
            catch (Exception ex)
            {
                ...
                logger.LogError(ex,
                    "Failed to send module notification: {NotificationId} → {Email}",
        ...
        logger.LogInformation(
            "Module notification batch completed: {NotificationId}, Module={Module}, Success={success}, Failed={failed}",
            evt.NotificationId, evt.TargetModule, successCount, failedCount);
    }

    private static string BuildCustomNotificationTemplate(CustomNotificationTemplateData template, string recipientName)
    {
        // Mesmo template usado no ToUsers
        ...
        <table role="presentation" width="600" cellspacing="0" cellpadding="0"
          ...
          <tr>
            <td style="font-size:12px; line-height:1.6; color:#9ca3af; padding-top:24px; border-top:1px solid #e5e7eb;">
              Esta é uma mensagem enviada pela coordenação do retiro.<br />
              Se tiver dúvidas, é só responder este e-mail.<br />
